<script lang="ts">
	...
	const laterTasks = [
		{ active: 'Finding what the buyer requested', done: 'Found what the buyer requested' },
		{ active: 'Checking what you can accept', done: 'Checked what you can accept' },
		{ active: 'Preparing your response', done: 'Prepared your response' }
	] as const;
	...
	let {
		...
		onFinished?: () => void;
		intake?: { active: string; done: string };
	} = $props();

	let tasks = $derived([intake, ...laterTasks]);
		...
		const run = async () => {
			try {
				for (let i = 0; i < tasks.length; i++) {
					activeTask = i;
					completed = i;
					await wait(i === 2 ? 2200 : 1800, signal);
					completed = i + 1;
